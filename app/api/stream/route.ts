...
import { getEventLen, getEventsFrom, getSession } from "../../../lib/store/sessions";
...
export async function GET(req: NextRequest) {
  ...
  let session: ConversationSession | null = await getSession(sessionId);
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        controller.enqueue(encoder.encode(`: prelude ${" ".repeat(8192)}\n\n`));
      } catch {}
      ...
      controller.enqueue(
        encoder.encode(
          `data: ${JSON.stringify({ type: "init", history: session.history, experts: session.experts, title: session.title })}\n\n`,
        ),
      );
      ...
      let index = 0;
      try {
        index = await getEventLen(sessionId);
      } catch {}
